...
| `sana_video/train.py` | Fine-tunes pretrained SANA-Video 2B on voxel "pseudo-video" volumes via Accelerate. `--mode lora` (PEFT adapters, default) or `--mode full` (all transformer weights). `max_frames` must be 4n+1 (Wan VAE). | `accelerate launch src/scripts/sana_video/train.py --manifest ... [--mode lora\|full] [--lora_rank 8] [--epochs 3] [--spatial_crop_size 512] [--max_frames 65] [--output_dir tmp/sana_video_ft]`. |
