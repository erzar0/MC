...
| `batching.py` | `batch_n(iterable, batch_count)` utility — splits an iterable into ~even batches (generators supported). | Imported helper. |
| `block_colors.py` | Shared block-ID <-> RGB palette: `load_block_states()` and `load_id2rgb()` (also returns air block IDs). Single source of truth for SANA-Video training and inference decoding. | Imported as `from src.block_colors import load_id2rgb`. |
...
| File | Description | Usage |
|------|-------------|-------|
| `pmc_data_crawler.py` | Selenium/undetected-chromedriver crawler over Planet Minecraft listings → `assets/pmc_data*.csv`. Resumable via cursor state JSON. | `python src/scripts/pmc_data_crawler.py [--platform 1\|2] [--years ...] [--output ...] [--chrome-version N]`. |
| `pmc_detail_crawler.py` | Deep per-project metadata crawler. Reads `assets/pmc_data.csv` → `assets/pmc_details.csv`, resumable via `pmc_details_crawl_state.json` (404s recorded there, input CSV untouched). | `python src/scripts/pmc_detail_crawler.py [--input ...] [--output ...] [--chrome-version N]`. |
...
| `llm_utils.py` | Shared vLLM helpers: `strip_thinking_tags()`, `make_vllm_client()`, `make_async_vllm_client()`. | Imported by `cleanse_descriptions.py` and `generate_vision_captions.py`. |
| `resumable_state.py` | `JsonStateStore` — flat `{id: entry}` JSON persistence base for resumable scripts. `DownloadState` (map_downloader) and `ProcessState` (process_downloads) subclass it; schemas unchanged. | Imported helper. |
| `generate_negative_buffer.py` | Aggregates block frequencies from all `metadata.json` files to build the negative-sampling buffer (`.pt`) consumed by `block2vec.py`. | `python src/scripts/generate_negative_buffer.py [--volumes-dir ...] [--output ...]`. |
...
| `visualize_embeddings.py` | Loads trained block embeddings + `block_states.txt`, reduces with PaCMAP/PCA, renders Plotly/matplotlib plots and an RGB LUT. | `python src/scripts/visualize_embeddings.py [--checkpoint ...]` (default: newest ckpt in `tmp/checkpoints`). |
