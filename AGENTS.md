...
| `sana_video/prepare_dataset.py` | Builds a JSONL manifest pairing each `.b2frame` volume with caption orientations from `tmp/processed_worlds/cleansed/`, plus each region's `height` (from `metadata.json` `y_range`) for bucketing. | `python src/scripts/sana_video/prepare_dataset.py [--cleansed_dir ...] [--output tmp/sana_video_manifest.jsonl]`. |
| `sana_video/dataset.py` | `MinecraftVideoDataset` — manifest-based dataset turning `.b2frame` block-ID volumes into RGB pseudo-video tensors (C, F, H, W) in [-1, 1] (Y-layers are frames, random 90° rotation + spatial crop + caption orientation). Regions are bucketed by content height (`4n+1`); `BucketBatchSampler` batches equal-height regions so no batch is air-padded. Requires a `height` field per manifest entry. | Imported by `sana_video/train.py`; not a standalone entrypoint. |
